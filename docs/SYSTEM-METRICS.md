...
啟用常駐模式後，選單列蒲公英圖示只在受管模型已載入且就緒時，使用 `/api/system/metrics` 的 GPU 快照換色：低於 50% 為綠色、50% 至低於 80% 為黃色、80% 以上為紅色。門檻與網頁底部 GPU 狀態列一致；滑鼠提示及輔助使用標籤同時提供百分比，不只依靠顏色傳達資訊。

指標回應額外提供 `model_loaded` 布林值，由 Runtime 的 `running`、`ready` 與非空模型欄位共同判定，不公開模型名稱、路徑或啟動參數。未載入、載入中、啟動失敗或停止後均恢復原本的系統單色圖示，不因其他程式使用 GPU 而變色；舊版回應未提供此旗標時亦維持單色。這項條件只影響選單列圖示，不改變底部系統資源狀態列的使用率與配色。

原生介面約每 3 秒讀取一次，隱藏主視窗或停在登入頁仍可更新，不觸發模型推論或額外的 GPU 採集。請求逾時、資料無效或 GPU 不可用時恢復系統單色圖示，提示 `GPU N/A`，不保留舊的高負載顏色。關閉常駐模式或結束程式時停止讀取。
