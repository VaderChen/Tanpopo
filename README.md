...
- Use OpenAI-compatible MLX SSE with early connection establishment and 10-second keep-alive comments. Disconnecting the client signals cancellation to the corresponding request; subsequent Prefill chunks and generation iterations stop after already-submitted GPU work drains.
- Support multiple MLX API users, with a current limit of four concurrent generation requests and per-request memory reservations. Context and memory checks can reduce the current Prefill chunk size without truncating input or changing saved settings.
