(() => {
  ...
  const rows = [
    ...
    ["選擇 mlx-server 的 GGUF 模型時自動開啟，原生 MLX 與 llama-server 不受影響。", "Automatically enables Fast GGUF when a GGUF model is selected for mlx-server. Native MLX models and llama-server are unaffected.", "mlx-server で GGUF モデルを選択したときに自動で有効化します。ネイティブ MLX モデルと llama-server には影響しません。", "mlx-server에서 GGUF 모델을 선택하면 자동으로 켭니다. 네이티브 MLX 모델과 llama-server에는 영향을 주지 않습니다."],
