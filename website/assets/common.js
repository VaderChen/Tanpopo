(() => {
  ...
  const rows = [
    ...
    ["清除快取", "Clear cache", "キャッシュを消去", "캐시 지우기"],
    ["確定要清除此模型的轉換快取嗎？原始 GGUF 與模型目錄不會刪除；下次以 MLX 載入時需要重新轉換。", "Clear this model's converted cache? The original GGUF and model directory will be kept; MLX must convert it again the next time it is loaded.", "このモデルの変換キャッシュを消去しますか？元の GGUF とモデルディレクトリは削除されません。次回 MLX で読み込む際に再変換が必要です。", "이 모델의 변환 캐시를 지우시겠습니까? 원본 GGUF와 모델 디렉터리는 유지되며 다음 MLX 로드 시 다시 변환해야 합니다."],
    ["正在清除快取…", "Clearing cache…", "キャッシュを消去しています…", "캐시를 지우는 중…"],
    ["轉換快取已清除", "Converted cache cleared", "変換キャッシュを消去しました", "변환 캐시를 지웠습니다"],
    ...
    ["本次啟動找不到可重用的轉換快取。轉換完成後會永久保存權重，避免下次重新轉換。", "No reusable conversion cache was found for this launch. Converted weights will be stored persistently to avoid reconversion next time.", "今回の起動で再利用可能な変換キャッシュが見つかりません。変換済みウェイトは永続保存され、次回の再変換を回避します。", "이번 실행에 재사용 가능한 변환 캐시가 없습니다. 변환된 가중치는 영구 저장되어 다음 실행에서 다시 변환하지 않습니다."],
    ["不建立快取直接啟動", "Start without creating a cache", "キャッシュを作成せず起動", "캐시를 만들지 않고 시작"],
    ["選擇「不建立快取直接啟動」不會寫入永久快取；GGUF 權重仍會在記憶體中即時準備為 MLX 可執行格式，下次啟動會再次處理。", "Starting without creating a cache does not write a persistent cache. GGUF weights are still prepared in memory for MLX execution and must be processed again on the next launch.", "「キャッシュを作成せず起動」を選ぶと永続キャッシュは書き込まれません。GGUF ウェイトは MLX 実行用にメモリ内で準備され、次回起動時に再処理されます。", "캐시를 만들지 않고 시작하면 영구 캐시를 기록하지 않습니다. GGUF 가중치는 MLX 실행을 위해 메모리에서 준비되며 다음 실행 때 다시 처리됩니다."],
    ...
    ["正在檢查轉換快取", "Checking conversion cache", "変換キャッシュを確認しています", "변환 캐시를 확인하는 중"],
    ...
    ["此模型需要轉換並建立永久快取；完成前請勿關閉程式。", "This model requires conversion and a persistent cache. Do not close the application until it finishes.", "このモデルは変換と永続キャッシュの作成が必要です。完了するまでアプリを閉じないでください。", "이 모델은 변환 및 영구 캐시 생성이 필요합니다. 완료될 때까지 앱을 종료하지 마세요."],
    ["正在建立轉換快取", "Saving conversion cache", "変換キャッシュを保存しています", "변환 캐시를 저장하는 중"],
    ["模型已完成轉換，正在寫入永久快取；後續載入可直接重用。", "Conversion is complete and the persistent cache is being written. Future loads can reuse it directly.", "変換が完了し、永続キャッシュへ書き込んでいます。次回以降は直接再利用できます。", "변환이 완료되어 영구 캐시에 저장하는 중입니다. 이후 로드에서는 바로 재사용할 수 있습니다."],
    ["正在載入轉換快取", "Loading conversion cache", "変換キャッシュを読み込んでいます", "변환 캐시를 불러오는 중"],
    ["已找到轉換完成的永久快取，本次不需要重新轉換。", "A completed persistent cache was found, so no reconversion is needed this time.", "変換済みの永続キャッシュが見つかったため、今回は再変換しません。", "완료된 영구 캐시를 찾았으므로 이번에는 다시 변환하지 않습니다."],
    ["正在直接載入模型", "Loading model directly", "モデルを直接読み込んでいます", "모델을 직접 불러오는 중"],
    ["本次不建立永久快取；正在從 GGUF 即時準備 MLX 執行所需的權重。", "No persistent cache will be created. Weights required for MLX execution are being prepared directly from GGUF.", "今回は永続キャッシュを作成しません。GGUF から MLX 実行に必要なウェイトを直接準備しています。", "이번에는 영구 캐시를 만들지 않습니다. GGUF에서 MLX 실행에 필요한 가중치를 직접 준비하는 중입니다."],
