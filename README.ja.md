...
新規インストールの既定 Runtime は、Tanpopo を実行するホストが macOS なら `mlx-server`、それ以外なら `llama-server` です。ブラウザーの OS では判定しません。保存済みの選択は維持され、既定値だけでモデルを自動起動することはありません。

```text
http://127.0.0.1:10082
```

新規インストールでは管理画面ログインが既定で無効です（`disable_authentication: true`）。保存済みの明示的な認証設定は維持されます。管理サービスは既定で全ネットワークインターフェースを待ち受けるため、LAN 公開や Reverse Proxy の有効化前に、システム設定で管理画面ログインを有効にして新しいパスワードを設定してください。初期の認証情報はネットワーク公開に使用しないでください。認証情報はローカルに保持され、後からログインを無効にする操作には確認が必要です。Model API の Access Key と IP 制限は別設定です。
...
[LICENSE.ja.md](LICENSE.ja.md)、[COMMERCIAL-LICENSE.md](COMMERCIAL-LICENSE.md)、[THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) を参照してください。Security report とローカル秘密情報の扱いは [SECURITY.md](SECURITY.md) に記載しています。
