(() => {
  ...
  const COMPATIBILITY = {
    ...
    "本次 Swift 測試共 33 項，其中 30 項通過、0 失敗，另 3 項需額外實體模型環境而略過； 4B、9B、27B 三組配對模型均完成原生 MLX、llama.cpp + GGUF 與三種 Fast GGUF 模式的固定 100 題、 生成速度、轉換快取及程序 RAM 量測。": "The Swift suite contained 33 tests: 30 passed, none failed, and three requiring physical-model fixtures were skipped. Paired 4B, 9B, and 27B models completed a fixed 100-question set, generation-speed, conversion-cache, and process-RAM measurements across native MLX, llama.cpp + GGUF, and three Fast GGUF modes.",
  ...
  const PERFORMANCE = {
    ...
    "以 4B、9B、27B 三組配對模型比較原生 MLX、llama.cpp + GGUF 與三種 MLX+GGUF 轉換策略。 報告如實列出生成速度、固定 100 題結果、轉換快取與程序 RAM，不納入載入效率。": "Paired 4B, 9B, and 27B models compare native MLX, llama.cpp + GGUF, and three MLX+GGUF conversion strategies. The report presents generation speed, fixed 100-question results, conversion cache, and process RAM without measuring loading efficiency.",
    "配對模型": "Paired models",
    ...
    "Fast GGUF 快取最小、速度最高": "Smallest Fast GGUF cache and highest speed",
    "RAM 最低、快取最大": "Lowest RAM, largest cache",
    "來源 4-bit block 與其他張量混用": "Mixes source 4-bit blocks with other tensors",
    "RAM 平均最低、快取最大": "Lowest average RAM, largest cache",
    ...
    "均排除": "is excluded",
    "三種 Fast GGUF 模式各自使用已完成的永久快取": "Each Fast GGUF mode uses its completed persistent cache",
    ...
    "本報告不量測載入效率；磁碟只列原始模型與永久轉換快取": "Loading efficiency is not measured; storage lists only the source model and persistent conversion cache",
