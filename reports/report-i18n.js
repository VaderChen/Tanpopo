(() => {
  ...
  const PERFORMANCE = {
    ...
    "本表保留整合 recurrent controls、tensor 契約驗證與新版快取前的基線數據；主系統完成這些修正後， fastGGUF 已成為多數可解析 GGUF 的建議預設。模型架構、量化方式、Tokenizer 與自訂 checkpoint 仍可能影響結果，因此不能把 fastGGUF 視為唯一執行手段；正式使用前應逐模型驗證速度與精確度， 並保留原生 MLX 或 llama+GGUF 作為相容路徑。": "This table retains baseline data collected before recurrent controls, tensor-contract validation, and the revised cache were integrated. With those fixes now in the main system, fastGGUF is the recommended default for most GGUF models that can be parsed. Model architecture, quantization, tokenizers, and custom checkpoints can still affect the result, so fastGGUF must not be treated as the only execution path. Validate speed and accuracy for each model before production use, and retain native MLX or llama+GGUF as compatibility paths.",
