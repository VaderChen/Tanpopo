(() => {
  ...
  const COMMON = {
    ...
    "MLX + Fast GGUF Mode 1": "MLX + Fast GGUF Mode 1",
    "MLX + Fast GGUF Mode 2": "MLX + Fast GGUF Mode 2",
    "MLX + Fast GGUF Mode 3": "MLX + Fast GGUF Mode 3",
    "Mode 1：均衡．預設": "Mode 1: Balanced · Default",
    "Mode 2：較高精度": "Mode 2: Higher Accuracy",
    "Mode 3：最快": "Mode 3: Fastest",
  ...
  const COMPATIBILITY = {
    ...
    "，共 23 項，其中 20 項通過、0 失敗， 另 3 項需額外實體模型環境的測試在一般回歸中略過；並以 4B、9B、27B 配對模型逐一完成 原生 MLX、MLX+GGUF 與 llama.cpp + GGUF 的載入、生成及固定 100 題品質驗證。": ". Of 23 checks, 20 passed with no failures and three requiring additional physical-model fixtures were skipped by the regular regression run. Paired 4B, 9B, and 27B models were also tested for loading, generation, and fixed-set 100-question quality across native MLX, MLX+GGUF, and llama.cpp + GGUF.",
    ...
    "Fast GGUF（.fgguf）": "Fast GGUF (.fgguf)",
    "Fast GGUF 以分片容器放在原始 GGUF 同一目錄；每個 tensor 依收益選擇 LZFSE 無損壓縮或 raw MMap 對齊。格式不是標準 GGUF，可由「移除 Fast GGUF」獨立移除。": "Fast GGUF is stored as sharded containers beside the source GGUF. Each tensor uses lossless LZFSE compression only when beneficial, otherwise raw MMap-aligned storage. This is not standard GGUF and can be removed independently with Remove Fast GGUF.",
    ...
    "本輪重新驗證：33 項程式回歸與固定題集": "Revalidated: 33 regression checks and fixed evaluation sets",
    "本次 Swift 測試共 33 項，其中 30 項通過、0 失敗，另 3 項需額外實體模型環境而略過； 4B、9B、27B 三組配對模型均完成原生 MLX、llama.cpp + GGUF 與三種 Fast GGUF 模式的固定 100 題、 生成速度、Fast GGUF 容量及程序 RAM 量測。": "The Swift suite contained 33 tests: 30 passed, none failed, and three requiring physical-model fixtures were skipped. Paired 4B, 9B, and 27B models completed a fixed 100-question set, generation-speed, Fast GGUF size, and process-RAM measurements across native MLX, llama.cpp + GGUF, and three Fast GGUF modes.",
    "目前尚未支援": "Not currently supported",
    "依 GGUF block metadata 沿用可表示的 K-Quant 來源 4-bit block；沿用 tensor 固定 group 32，其餘張量自動使用 group 64。": "Reuses representable source K-Quant 4-bit blocks according to GGUF block metadata. Reused tensors use group 32, while other tensors automatically use group 64.",
    "預設模式": "Default mode",
    "將低位元來源重新量化為 MLX INT8，策略自動使用 group 64；浮點與小型參數仍依原有型別規則處理。": "Requantizes low-bit sources to MLX INT8 with automatic group 64. Floating-point and small parameters retain their existing type rules.",
    "將低位元來源重新量化為 MLX INT4，策略固定使用 group 32，不受外部 group size 設定覆寫。": "Requantizes low-bit sources to MLX INT4 with a fixed group 32 that external group-size settings cannot override.",
    "本輪五種模式結果依序為原生 MLX、llama.cpp + GGUF、Mode 1、Mode 2、Mode 3；可載入與輸出品質是不同驗證項目。": "Results are ordered as native MLX, llama.cpp + GGUF, Mode 1, Mode 2, and Mode 3. Loadability and output quality are separate validation items.",
    "五種模式，固定 100 題": "Five modes, fixed 100 questions",
    "MLX／GGUF 混合量化": "MLX / GGUF mixed quantization",
    "使用 MMap 技術降低 RAM 用量，原生 MLX 目前尚未支援；需要重新量化的 tensor 仍可能產生常駐 MLX 權重。": "MMap reduces RAM usage and is not currently supported by native MLX. Tensors requiring requantization may still create resident MLX weights."
  };
  ...
  const PERFORMANCE = {
    ...
    "以 4B、9B、27B 三組配對模型重新比較原生MLX檔案未轉換、fastGGUF 開啟、fastGGUF 關閉及 llama.cpp + GGUF。速度接近原生MLX檔案未轉換 並不足夠；本報告也以固定 100 題驗證輸出品質。": "Three paired 4B, 9B, and 27B models compare native MLX, fastGGUF on, fastGGUF off, and llama.cpp + GGUF. Approaching native MLX speed is not enough; a fixed 100-question set also validates output quality.",
    "測試摘要": "Test summary",
    "3 組": "3 pairs",
    ...
    "llama.cpp + GGUF 在三模型都維持有效答案與合理精度，且不需支付 MLX 重新量化的載入成本。": "llama.cpp + GGUF maintained valid answers and reasonable accuracy for all three models without the MLX requantization load cost.",
    ...
    "磁碟分開列出原始模型與 Fast GGUF；RAM 每 200 ms 取樣，列出程序生命週期峰值與生成期間平均值。": "Storage lists the source model and Fast GGUF separately. RAM is sampled every 200 ms, reporting process-lifetime peak and generation-period average.",
    "原始模型": "Source model",
    "Fast GGUF": "Fast GGUF",
    "RAM 峰值": "Peak RAM",
    "RAM 平均": "Average RAM",
    "無 Fast GGUF": "No Fast GGUF",
    "RAM 最低，Fast GGUF 最大": "Lowest RAM, largest Fast GGUF",
    "各方面表現均衡": "Balanced across measured dimensions",
    "GGUF 策略中 Fast GGUF 最小": "Smallest Fast GGUF among GGUF strategies",
    ...
    "MLX+GGUF 速度量測使用已完成的 Fast GGUF": "MLX+GGUF speed measurements use completed Fast GGUF files",
    ...
    "以 4B、9B、27B 三組配對模型比較原生 MLX、llama.cpp + GGUF 與三種 MLX+GGUF 轉換策略。 報告如實列出生成速度、固定 100 題結果、Fast GGUF 容量與程序 RAM，不納入載入效率。": "Paired 4B, 9B, and 27B models compare native MLX, llama.cpp + GGUF, and three MLX+GGUF conversion strategies. The report presents generation speed, fixed 100-question results, Fast GGUF size, and process RAM without measuring loading efficiency.",
    "配對模型": "Paired models",
    "每組執行模式": "Modes per model",
    "5 種": "5 modes",
    "45 次": "45 runs",
    "1,500 次": "1,500 questions",
    "15 組配置在同一台主機串行執行；同一時間只啟動一個模型 Runtime。": "All 15 configurations ran serially on the same host, with only one model runtime active at a time.",
    "與系統 CPU 取樣確認沒有其他 mlx-server／llama-server；CPU idle 維持 80.5%～83.8%，系統未回報溫控或效能限制。27B llama.cpp + GGUF 的波動結果在相同條件下重測，正式吞吐採三次中位數。": "and system CPU sampling confirmed no other mlx-server or llama-server process was active. CPU idle remained at 80.5%-83.8%, with no thermal or performance limits reported. The variable 27B llama.cpp + GGUF result was rerun under the same conditions; formal throughput uses the median of three runs.",
    "Group 完全由模式與來源 tensor metadata 自動決定，不依模型名稱，也不需手動指定。": "Group sizing is determined automatically by the mode and source tensor metadata, without model-name rules or manual specification.",
    "llama.cpp 原生 GGUF": "Native llama.cpp GGUF",
    "直接使用來源 GGUF 量化": "Uses source GGUF quantization directly",
    "GGUF Runtime 對照": "GGUF runtime reference",
    "K-Quant 沿用來源 4-bit block；其他張量 group 64": "K-Quant reuses source 4-bit blocks; other tensors use group 64",
    "均衡．預設": "Balanced · Default",
    "低位元來源重新量化為 INT8／group 64": "Low-bit sources requantized to INT8 / group 64",
    "較高精度": "Higher accuracy",
    "低位元來源重新量化為 INT4／group 32": "Low-bit sources requantized to INT4 / group 32",
    "最快": "Fastest",
    "Mode 1 沿用的 K-Quant tensor 由來源格式保留 32 元素 sub-block；Mode 2 自動使用 group 64，Mode 3 固定使用 group 32。 三種模式均使用相同 recurrent controls，且各自建立獨立 Fast GGUF。": "Mode 1 preserves source-format 32-element sub-blocks for reused K-Quant tensors. Mode 2 automatically uses group 64, while Mode 3 fixes group 32. All three modes use the same recurrent controls and create separate Fast GGUF files.",
    "比較 llama.cpp + GGUF": "Compared with llama.cpp + GGUF",
    "MMLU 固定 100 題；每個模型的五種模式使用完全相同題目。": "MMLU uses a fixed 100-question set; all five modes for each model use identical questions.",
    "原樣結果": "Raw result",
    "排除截斷": "Excluding truncation",
    "共同未截斷題組": "Common untruncated set",
    "無法解析": "Unparseable",
    "所有": "Every",
    "均視為截斷並排除分母，即使截斷前已出現可解析選項也不計分； 模型自行停止卻未作答仍算錯。本輪 1,500 次問答共有 3 次截斷，排除截斷後共 1,497 次可評分。 「共同未截斷題組」只比較同一模型下五種模式都未截斷的題目。": "is treated as truncated and excluded from the denominator, even when a parseable option appeared before truncation. A model that stops without answering remains incorrect. Of 1,500 responses, three were truncated, leaving 1,497 evaluable responses. The common untruncated set includes only questions untruncated in all five modes for the same model.",
    "直接執行來源 GGUF": "Runs the source GGUF directly",
    "RAM 最低": "Lowest RAM",
    "Mode 1 接近": "Close to Mode 1",
    "Fast GGUF 最小、速度最高": "Smallest Fast GGUF and highest speed",
    "RAM 最低、Fast GGUF 最大": "Lowest RAM, largest Fast GGUF",
    "來源 4-bit block 與其他張量混用": "Mixes source 4-bit blocks with other tensors",
    "RAM 平均最低、Fast GGUF 最大": "Lowest average RAM, largest Fast GGUF",
    "使用 MMap 技術降低 RAM 用量，原生 MLX 目前尚未支援。表中為程序 RSS，因此不等於邏輯權重總量或整台主機的統一記憶體使用量。": "MMap reduces RAM usage and is not currently supported by native MLX. The table reports process RSS, not logical weight size or total host unified-memory use.",
    "本輪三種 Fast GGUF 模式均使用相同的 recurrent controls 設定，避免額外變因。": "All three Fast GGUF modes use the same recurrent-controls setting to avoid an additional variable.",
    "MLX + Fast GGUF Mode 1、Mode 2 與 Mode 3 均傳入": "MLX + Fast GGUF Modes 1, 2, and 3 all receive",
    "只接受獨立 A、B、C、D；所有": "Only standalone A, B, C, or D is valid; every",
    "均排除": "is excluded",
    "三種 Fast GGUF 模式各自使用已完成的 Fast GGUF": "Each Fast GGUF mode uses its completed Fast GGUF file",
    "Group 由 Mode 1／2／3 自動決定，未手動指定": "Group sizing is selected automatically by Modes 1/2/3 without manual specification",
    "本輪涵蓋 4B Q4_0、9B Q4_K_M 與 27B 混合量化來源": "This round covers 4B Q4_0, 9B Q4_K_M, and a 27B mixed-quantization source",
    "每個模型均完成五種模式、固定 100 題與三次正式速度量測": "Each model completed five modes, a fixed 100-question set, and three formal speed runs",
    "本報告不量測載入效率；磁碟只列原始模型與 Fast GGUF": "Loading efficiency is not measured; storage lists only the source model and Fast GGUF",
    "· 每題結果：": "· Per-question results:",
    "· 速度與資源原始彙總：": "· Raw speed and resource summary:",
    "五種模式排除截斷後為 68.0%～73.0%；原生 MLX 為 81.609 tok/s，三種 Fast GGUF 為 71.301～75.137 tok/s。": "Across five modes, accuracy excluding truncation was 68.0%-73.0%. Native MLX measured 81.609 tok/s, while the three Fast GGUF modes measured 71.301-75.137 tok/s.",
    "五種模式排除截斷後為 77.0%～78.8%；原生 MLX 為 48.438 tok/s，三種 Fast GGUF 為 27.335～43.177 tok/s。": "Across five modes, accuracy excluding truncation was 77.0%-78.8%. Native MLX measured 48.438 tok/s, while the three Fast GGUF modes measured 27.335-43.177 tok/s.",
    "五種模式排除截斷後為 82.0%～89.0%；原生 MLX 為 15.088 tok/s，三種 Fast GGUF 為 8.343～13.482 tok/s。": "Across five modes, accuracy excluding truncation was 82.0%-89.0%. Native MLX measured 15.088 tok/s, while the three Fast GGUF modes measured 8.343-13.482 tok/s.",
    "三組模型的速度、精確度、Fast GGUF 與 RAM 分布並不相同；結果只代表本次模型、Runtime 與測試條件，其他架構、量化格式或 checkpoint 需另行量測。": "The three models show different speed, accuracy, Fast GGUF size, and RAM distributions. Results apply only to these models, runtime, and test conditions; other architectures, quantization formats, or checkpoints require separate measurement."
  };
