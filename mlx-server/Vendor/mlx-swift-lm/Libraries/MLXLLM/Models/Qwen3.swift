...
public class Qwen3ModelInner: Module {
    ...
    /// DFlash 專用 forward：依照 checkpoint 宣告順序擷取指定 layer 的輸出。
    /// 一般生成仍使用上方零額外配置的 fast path。
    func callAsFunction(
        _ inputs: MLXArray,
        cache: [KVCache]?,
        captureLayerIDs: [Int]
        ...
        return (norm(h), concatenated(captured, axis: -1))
    }
}

public class Qwen3Model: Module, LLMModel, KVCacheDimensionProvider, DFlashTargetModel {
    ...
    public func dflashForward(
        ...
        var (hidden, captured) = model(
            inputs,
            cache: cache,
            captureLayerIDs: captureLayerIDs
        )
        if let lmHead {
            hidden = lmHead(hidden)
        } else {
            hidden = model.embedTokens.asLinear(hidden)
        }
        return DFlashTargetOutput(logits: hidden, hiddenStates: captured)
    }

    public var dflashModelType: String { "qwen3" }
    ...
    public var dflashSupportsNonTrimmableCacheRollback: Bool { false }
