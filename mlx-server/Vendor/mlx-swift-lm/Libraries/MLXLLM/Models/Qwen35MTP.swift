...
public final class Qwen35MTPDraftModel: Module, StatefulMTPDrafterModel {
    ...
    public func validate(target: any LanguageModel) throws {
        ...
        if let target = target as? Qwen35Model {
            targetConfiguration = target.languageModel.configuration
        } else if let target = target as? Qwen35TextModel {
            targetConfiguration = target.configuration
        } else {
            throw MTPCompatibilityError.unsupportedTarget(String(describing: type(of: target)))
        }

        let checks: [(String, Int, Int)] = [
            ("hidden_size", configuration.hiddenSize, targetConfiguration.hiddenSize),
            ("vocab_size", configuration.vocabularySize, targetConfiguration.vocabularySize),
            ("num_hidden_layers", configuration.hiddenLayers, targetConfiguration.hiddenLayers),
            ("num_attention_heads", configuration.attentionHeads, targetConfiguration.attentionHeads),
            ("num_key_value_heads", configuration.kvHeads, targetConfiguration.kvHeads),
            ("full_attention_interval", configuration.fullAttentionInterval,
                targetConfiguration.fullAttentionInterval),
        ]
        if let mismatch = checks.first(where: { $0.1 != $0.2 }) {
            throw MTPCompatibilityError.incompatibleConfiguration(
                "\(mismatch.0) draft=\(mismatch.1) target=\(mismatch.2)")
        }
    }
