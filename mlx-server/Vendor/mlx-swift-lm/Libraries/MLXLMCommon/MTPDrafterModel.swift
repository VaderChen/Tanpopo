...
public protocol MTPDrafterModel: BaseLanguageModel {
    /// Verifies that a target exposes the architecture and tensor dimensions
    /// expected by this drafter. Implementations should compare structural
    /// configuration rather than repository or model display names.
...
extension MTPDrafterModel {
    public func validate(target _: any LanguageModel) throws {}
...
public enum MTPCompatibilityError: LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .unsupportedTarget(let type):
            "MTP draft 不支援目前的 target 架構：\(type)"
        case .incompatibleConfiguration(let detail):
            "MTP draft 與 target 設定不相容：\(detail)"
        }
    }
}
...
/// layer type here. The tensor length can be capped by a rotating cache, so
/// it cannot always be used as the absolute continuation position.
public let mtpSharedKVOffsetsKey =
    LMOutput.Key<[String: Int]>("mtp.sharedKVOffsets")

/// Target writes optional multimodal position-delta state here so a drafter
/// can continue the same RoPE position sequence as the target.
...
/// tokens. Hybrid targets use it to rewind a rejected MTP proposal without
/// replaying the committed bonus token.
public let mtpCacheCheckpointIndexKey =
    LMOutput.Key<Int>("mtp.cacheCheckpointIndex")

/// Records which cache entry produced each shared-K/V snapshot. This keeps
/// snapshot reconciliation exact for mixed global and sliding caches.
