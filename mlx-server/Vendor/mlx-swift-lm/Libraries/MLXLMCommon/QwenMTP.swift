...
package func qwenMTPSanitizeWeights(
    ...
    // Hugging Face 的完整 Target checkpoint 將 MTP 權重放在 `mtp.*`；獨立
    // MTP Draft checkpoint 則通常從 `fc.*`、`layers.*` 起算。模型本身的
    // Module path 固定為 `mtp`，因此只在獨立格式缺少前綴時統一補上。
    let namespacedWeights: [String: MLXArray]
    if weights.keys.contains(where: { $0.hasPrefix("mtp.") }) {
        namespacedWeights = weights
    } else {
        namespacedWeights = Dictionary(
            uniqueKeysWithValues: weights.map { ("mtp.\($0.key)", $0.value) })
    }
    var sanitized = namespacedWeights.filter { key, _ in key.hasPrefix("mtp.") }
